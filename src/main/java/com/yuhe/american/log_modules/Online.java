...
import com.yuhe.american.db.statics.OnlineDB;
...
public class Online extends AbstractLogModule {
	...
	/**
	 * 如果前10分钟有数据但是前5分钟没有数据，就认为前5分钟数据 有问题，直接用前10分钟的数据记录成前5分钟的数据
	 * 
	 * @param platformID
	 * @param hostID
	 * @param time
	...
	public Map<String, List<Map<String, String>>> execute4Kafka(JSONObject json, Map<String, String> staticsHosts) {
		...
		while (it.hasNext()) {
			...
			db.batchInsert(platformID, platformResult);
			for (Map<String, String> result : platformResult) {
				String tTime = result.get("Time");
				String tHostID = result.get("HostID");
				// 检查5分钟前数据是否正常
				checkPreNumErro(platformID, tHostID, tTime);
			}
		}
		return platformResults;
	}
