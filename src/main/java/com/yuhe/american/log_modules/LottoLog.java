 ...
 *
 */
public class LottoLog extends AbstractLogModule {

	private static final String[] LOG_COLS = { "Uid", "Urs", "Name", "Rate", "PayGold", "ExchangeGold", "LottoTime" };
	private static final String[] DB_COLS = { "HostID", "Uid", "Urs", "Name", "Rate", "PayGold", "ExchangeGold",
			"LottoTime", "Time" };
	private static String TBL_NAME = "tblLottoLog";
			...
			if (json != null) {
				...
				if (StringUtils.isNotBlank(message) && hostMap.containsKey(hostID)) {
					...
					for (String col : LOG_COLS) {
						...
						if (col.equals("LottoTime")) {
							value = DateUtils2.getTimeStr(Integer.parseInt(value));
						}
						map.put(col, value);
					}
