...
public class MountLevelLog extends AbstractLogModule {

	private static final String[] LOG_COLS = { "Uid", "Name", "MountID", "TransID", "OrgEvoLevel", "NewEvoLevel",
			"OrgExp", "NewExp", "Consumes" };
	private static final String[] DB_COLS = { "HostID", "Uid", "Name", "MountID", "TransID", "OrgEvoLevel",
			"NewEvoLevel", "OrgExp", "NewExp", "Consumes", "Time" };
	private static String TBL_NAME = "tblMountLevelLog";
			...
			if (json != null) {
				...
				if (StringUtils.isNotBlank(message) && hostMap.containsKey(hostID)) {
					...
					for (String col : LOG_COLS) {
						...
						if (col.equals("TransID") && value.equals("")) {
							value = "0"; // 默认是0
						}
						map.put(col, value);
					}
